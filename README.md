...
- Live orientation lines summarize current objective + decision rule + explicit "why now" rationale + mode + a 10-second quick-summary line + a dynamic workflow compass + a three-click path + operator sequence + a first-minute plan + "Step X of 4" position, include section-specific goal + action lines, show an at-a-glance board snapshot + workflow progress percentage, and provide one-click **Do this now** + **Next step** buttons
